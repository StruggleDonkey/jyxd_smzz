...
public class NursingRecordController {
     ...
     * @return code=score status=1
     */
    @RequestMapping(value = "/getListByStart")
    @ResponseBody
    public String getListByStart(@RequestBody(required = false) Map<String, Object> map) {
        JSONObject json = new JSONObject();
        json.put("code", HttpCode.FAILURE_CODE.getCode());
        json.put("data", new ArrayList<>());
        json.put("msg", "暂无数据");
        try {
            if (map.containsKey("patientId") && StringUtils.isNotEmpty(map.get("patientId").toString())) {
                ...
                List<Map<String, Object>> list = nursingRecordService.getListByStartTime(map);
                if (list != null && list.size() > 0) {
                    json.put("data", JSONArray.fromObject(list));
                    json.put("code", HttpCode.OK_CODE.getCode());
                    json.put("msg", "查询成功");
